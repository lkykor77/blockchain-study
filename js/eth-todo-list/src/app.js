App = {
  ...
  renderTasks: async () => {
    ...
    const taskCount = await App.todoList.taskCount(); //TodoList.sol에서 만든 taskCount를 호출함
    const $taskTemplate = $(".taskTemplate");

    // Render out each task with a new task template
    for (var i = 1; i <= taskCount; i++) {
      // Fetch the task data from the blockchain
      const task = await App.todoList.tasks(i); //TodoList.sol에 tasks를 하나씩 불러옴
      const taskId = task[0].toNumber(); //불러온 task의 정보
